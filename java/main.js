...
// (UI Preferences like sidebar collapse are safe to keep in localStorage)
...
function toggleMobileSidebar() {
    ...
    if (mobileSidebar) mobileSidebar.classList.toggle('show');
    if (overlay) overlay.classList.toggle('show');
}

if (mobileToggle) mobileToggle.addEventListener('click', toggleMobileSidebar);
if (landingMobileMenuToggle) landingMobileMenuToggle.addEventListener('click', (e) => { e.stopPropagation(); toggleMobileSidebar(); });
...
// 4. Theme Toggle Logic (UI Preference)
...
if (themeToggle) themeToggle.addEventListener('click', () => applyTheme(!body.classList.contains('light-mode')));
if (settingsThemeToggle) settingsThemeToggle.addEventListener('change', (e) => applyTheme(!e.target.checked));
...
if (notifList) {
    notifList.addEventListener('click', (e) => {
        ...
        if (optBtn) {
            e.stopPropagation();
            if (optBtn.classList.contains('mark-read-opt')) item.classList.remove('unread');
            else if (optBtn.classList.contains('mark-unread-opt')) item.classList.add('unread');
...
document.addEventListener('click', (e) => {
    document.querySelectorAll('.notif-options.show').forEach(opt => opt.classList.remove('show'));
    if (notifDropdown && notifToggle && !notifDropdown.contains(e.target) && !notifToggle.contains(e.target)) notifDropdown.classList.remove('show');
    if (profileDropdown && profileToggle && !profileDropdown.contains(e.target) && !profileToggle.contains(e.target)) profileDropdown.classList.remove('show');
});

// 7. GLOBAL TOAST NOTIFICATION LOGIC
...
// 8. GLOBAL SLIDESHOW LOGIC (Restored)
